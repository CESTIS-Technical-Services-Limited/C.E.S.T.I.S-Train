...
| D4 | Fee income vs cashbook | `cestiSchoolFeePayments` | `cestis_quarter_*` deposits | **no linkage** (verified: zero cross-references); deposits in practice = subvention only. **Reclassified 2026-08-05 (decision D11): intentional — the cashbook is a separate book from school fees; not a defect, no join will be built** | n/a — by design | — |

...
| ~~Sibling apps store passwords in plaintext.~~ **FIXED (2026-07-16).** | — | Ported the PBKDF2 + per-user-salt module into all three stores: `cestiUsers` (School.Fee), `dashboardUsers` (Payslip) and the shared `cestisStaffMembers` (Staff.Clock.in **+** the LMS time-clock, converted in lockstep). Login/verify is hash-aware (pbkdf2 + legacy plaintext), new/reset/edited passwords hash at rest, and plaintext upgrades lazily on next login — no bulk migration, so a rollout window can't lock anyone out. Cross-subsystem interop for the shared store was proven both directions in a headless browser. No user credential is stored in plaintext at rest anymore. |

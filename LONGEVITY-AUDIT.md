...
| Risk | Severity | Note |
|---|---|---|
| **No `If-Match`/generation precondition on the shared Drive file; 5 s auto-save has no conflict check, and there is no periodic pull.** Two connected devices can overwrite each other (last-writer-wins). | HIGH (data loss window) | Mitigated by additive read-merges, the "last saved by" prompt on manual save, and the WebRTC nudge-pull — but not eliminated. Right fix: precondition the PATCH on the file's generation, and add a periodic pull+merge timer. |
...
*Residual cross-device conflict handling (section 3) is the main remaining longevity work; the data-durability and credential foundations are sound.*
